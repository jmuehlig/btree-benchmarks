...
#define bwt_printf(fmt, ...)                                                                                           \
    ...
    {                                                                                                                  \
        if (print_flag == false)                                                                                       \
            break;                                                                                                     \
        fprintf(stderr, "%-24s(%8lX): " fmt, __FUNCTION__, std::hash<std::thread::id>()(std::this_thread::get_id()),   \
                ##__VA_ARGS__);                                                                                        \
        fflush(stdout);                                                                                                \
    } while (0);
...
static_assert(INNER_DELTA_CHAIN_LENGTH_THRESHOLD >= 1 && LEAF_DELTA_CHAIN_LENGTH_THRESHOLD >= 1,
...
static constexpr int LEAF_NODE_SIZE_UPPER_THRESHOLD = 64;
static constexpr int LEAF_NODE_SIZE_LOWER_THRESHOLD = 16;

// Make sure the inner node size is legal
static_assert(INNER_NODE_SIZE_UPPER_THRESHOLD > (INNER_NODE_SIZE_LOWER_THRESHOLD * 2),
              "Inner node split threshold must be no smaller than 2 times"
              " of the merge threshold");

// Make sure the leaf node size is legal
static_assert(LEAF_NODE_SIZE_UPPER_THRESHOLD > (LEAF_NODE_SIZE_LOWER_THRESHOLD * 2),
...
#define INC_COUNTER(name, value)                                                                                       \
    do                                                                                                                 \
    {                                                                                                                  \
        GetCurrentGCMetaData()->counters[GCMetaData::CounterType::name] += value;                                      \
    } while (false);
#else
#define INC_COUNTER(name, value)                                                                                       \
    do                                                                                                                 \
...
public:
    ...
    inline T load() const { return data; }

    inline void store(const T &pdata) { data = pdata; }

    inline bool compare_exchange_strong(const T old_val, T &new_val)
    {
        // if(data == old_val) {
...
#define InnerInlineAllocateOfType(T, node_p, ...)                                                                      \
    (static_cast<T *>(new (InnerNode::InlineAllocate(&node_p->GetLowKeyPair(), sizeof(T))) T{__VA_ARGS__}))
#else
...
#define LeafInlineAllocateOfType(T, node_p, ...)                                                                       \
    (static_cast<T *>(new (LeafNode::InlineAllocate(&node_p->GetLowKeyPair(), sizeof(T))) T{__VA_ARGS__}))
#else
// If preallocation is not enabled this is simply an operator new
...
public:
     ...
     */
    template <typename DataType, size_t Alignment> class PaddedData
    {
    public:
    ...
    static_assert(sizeof(PaddedGCMetadata) % PaddedGCMetadata::ALIGNMENT == 0, "class PaddedGCMetadata size does"
...
public:
    ...
    void PrepareThreadLocal()
    {
        ...
        // for doing alignment
        original_p = static_cast<unsigned char *>(malloc(sizeof(PaddedGCMetadata) * thread_num + CACHE_LINE_SIZE));
        assert(original_p != nullptr);
...
public:
    ...
    uint64_t SummarizeGCEpoch()
    {
        ...
        {
            // Note: std::min pass a const & of into the function. We need to first copy the shared
            // GetGCMetaData(i)->last_active_epoch into a local variable before calling std::min. Otherwise we will have
            // a Heisenbug where std::min first check which one is smaller, and before it returns, other thread modify
            // the variable and we actually return the larger one.
...
public:
     ...
     */
    template <typename T> static constexpr T LargerOne(const T &a, const T &b) { return a > b ? a : b; }
    ...
    public:
        ...
        inline Context(const KeyType &p_search_key)
            :
              ...
              abort_counter{0}, current_level{-1},
    ...
    public:
        ...
        NodeMetaData(const KeyNodeIDPair *p_low_key_p, const KeyNodeIDPair *p_high_key_p, NodeType p_type, int p_depth,
                     int p_item_count)
            : low_key_p{p_low_key_p}, high_key_p{p_high_key_p}, type{p_type}, depth{static_cast<short>(p_depth)},
    ...
    public:
         ...
         */
        inline NodeMetaData &GetMetaData() { return metadata; }
    ...
    public:
        ...
        LeafDataNode(const KeyValuePair &p_item, NodeType p_type, const BaseNode *p_child_node_p,
                     ...
                     const KeyNodeIDPair *p_high_key_p, int p_depth, int p_item_count)
            : DeltaNode{p_type, p_child_node_p, p_low_key_p, p_high_key_p, p_depth, p_item_count}, item{p_item},
    ...
    public:
        ...
        LeafInsertNode(const KeyType &p_insert_key, const ValueType &p_value, const BaseNode *p_child_node_p,
            ...
            : LeafDataNode{std::make_pair(p_insert_key, p_value), NodeType::LeafInsertType, p_child_node_p,
                           p_index_pair, &p_child_node_p->GetLowKeyPair(), &p_child_node_p->GetHighKeyPair(),
    ...
    public:
        ...
        LeafDeleteNode(const KeyType &p_delete_key, const ValueType &p_value, const BaseNode *p_child_node_p,
                       std::pair<int, bool> p_index_pair)
            : LeafDataNode{std::make_pair(p_delete_key, p_value), NodeType::LeafDeleteType, p_child_node_p,
                           p_index_pair, &p_child_node_p->GetLowKeyPair(), &p_child_node_p->GetHighKeyPair(),
                           p_child_node_p->GetDepth() + 1,
    ...
    public:
         ...
         * Constructor
         */
        LeafUpdateNode(const KeyType &p_update_key, const ValueType &p_old_value, const ValueType &p_new_value,
                       const BaseNode *p_child_node_p, std::pair<int, bool> p_index_pair)
    ...
    public:
         ...
         * the current node
         */
        LeafSplitNode(const KeyNodeIDPair &p_insert_item, const BaseNode *p_child_node_p,
    ...
    public:
        ...
        LeafMergeNode(const KeyType &p_merge_key, const BaseNode *p_right_merge_p, NodeID p_deleted_node_id,
            ...
            : DeltaNode{NodeType::LeafMergeType, p_child_node_p, &p_child_node_p->GetLowKeyPair(),
              ...
              delete_item{p_merge_key, p_deleted_node_id}, right_merge_p{p_right_merge_p}
        {
        }
    };
    ...
    public:
        ...
        InnerDataNode(const KeyNodeIDPair &p_item, NodeType p_type, const BaseNode *p_child_node_p, int p_location,
                      const KeyNodeIDPair *p_low_key_p, const KeyNodeIDPair *p_high_key_p, int p_depth,
                      int p_item_count)
            : DeltaNode{p_type, p_child_node_p, p_low_key_p, p_high_key_p, p_depth, p_item_count}, item{p_item},
              location{p_location}
        {
    ...
    public:
        ...
        InnerInsertNode(const KeyNodeIDPair &p_insert_item, const KeyNodeIDPair &p_next_item,
                        const BaseNode *p_child_node_p, int p_location)
    ...
    public:
        ...
        InnerDeleteNode(const KeyNodeIDPair &p_delete_item, const KeyNodeIDPair &p_prev_item,
                        const KeyNodeIDPair &p_next_item, const BaseNode *p_child_node_p, int p_location)
            : InnerDataNode{p_delete_item,
                            ...
                            p_child_node_p->GetDepth() + 1,
                            p_child_node_p->GetItemCount() - 1},
              prev_item{p_prev_item}, next_item{p_next_item}
        {
        }
    };
    ...
    public:
         ...
         * Constructor
         */
        InnerSplitNode(const KeyNodeIDPair &p_insert_item, const BaseNode *p_child_node_p,
                       const BaseNode *p_split_node_p)
    ...
    public:
         ...
         */
        inline bool IsLeafLevel() const { return node_p->IsOnLeafDeltaChain(); }
    };
     ...
     */
    template <size_t chunk_size> class AllocationMeta
    {
    private:
     ...
     * not take any space
     */
    template <size_t chunk_size, typename ExtraDataType> class ElasticNode : public BaseNode
    {
    public:
    ...
    public:
        ...
        ElasticNode(NodeType p_type, int p_depth, int p_item_count, const KeyNodeIDPair &p_low_key,
            ...
            : BaseNode{p_type, &low_key, &high_key, p_depth, p_item_count}, low_key{p_low_key}, high_key{p_high_key},
              end{start}, extra_data{}
        {
        }
    ...
    public:
        ...
        inline static EN *Get(int byte_size, // Bytes to be allocated
                              NodeType p_type, int p_depth, int p_item_count, const KeyNodeIDPair &p_low_key,
                              const KeyNodeIDPair &p_high_key)
        {
            // Allocte memory for
            ...
            //                         ^                        ^
            //                       Limit                     Tail
            char *alloc_base = new char[sizeof(EN) + byte_size + chunk_size + sizeof(AM)];
            ...
            // AllocationMeta
            new (reinterpret_cast<AM *>(alloc_base)) AM{alloc_base + chunk_size + sizeof(AM), alloc_base + sizeof(AM)};

            // The first chunk_size byte is used by class AllocationMeta
            // and chunk data
            ElasticNode *node_p = reinterpret_cast<ElasticNode *>(alloc_base + sizeof(AM) + chunk_size);

            // Call placement new to initialize all that could be initialized
            new (node_p) EN{p_type, p_depth, p_item_count, p_low_key, p_high_key};
        ...
        static EN *GetNodeHeader(const KeyNodeIDPair *low_key_p)
        {
            ...
            return reinterpret_cast<EN *>(reinterpret_cast<uint64_t>(low_key_p) - low_key_offset);
        }
        ...
        {
            return reinterpret_cast<AM *>(reinterpret_cast<uint64_t>(node_p) - chunk_size - sizeof(AM));
        }
         ...
         * be wrong.
         */
        static void GetAllocationStatistics(const KeyNodeIDPair *low_key_p, size_t *alloc_size_p, size_t *used_size_p)
        {
    ...
    using InnerBaseType = ElasticNode<INNER_DELTA_CHAIN_LENGTH_THRESHOLD * sizeof(InnerDeltaNodeUnion), NodeID *>;
    // This is the base type of leaf nodes
    using LeafBaseType = ElasticNode<LEAF_DELTA_CHAIN_LENGTH_THRESHOLD * sizeof(LeafDeltaNodeUnion), char[0]>;
#else
    // This is the base type of inner nodes
    using InnerBaseType = ElasticNode<0UL, NodeID *>;
    // This is the base type of leaf nodes
    using LeafBaseType = ElasticNode<0UL, char[0]>;
#endif

public:
#ifdef BWTREE_PREALLOCATION
    static const size_t INNER_PREALLOCATION_SIZE = INNER_DELTA_CHAIN_LENGTH_THRESHOLD * sizeof(InnerDeltaNodeUnion);
    static const size_t LEAF_PREALLOCATION_SIZE = LEAF_DELTA_CHAIN_LENGTH_THRESHOLD * sizeof(LeafDeltaNodeUnion);
#else
...
private:
    ...
    public:
         ...
         */
        inline static InnerNode *Get(int p_depth, int p_item_count, const KeyNodeIDPair &p_low_key,
                                     const KeyNodeIDPair &p_high_key)
        {
            // This is the byte size of the KeyType and NodeID array
            size_t inner_node_size = (sizeof(KeyType) + sizeof(NodeID)) * p_item_count;

            InnerNode *inner_node_p = static_cast<InnerNode *>(
                InnerBaseType::Get(inner_node_size, NodeType::InnerType, p_depth, p_item_count, p_low_key, p_high_key));
            ...
            // node ID array
            inner_node_p->extra_data = reinterpret_cast<NodeID *>(inner_node_p->start + sizeof(KeyType) * p_item_count);
         ...
         */
        inline KeyType *KeyBegin() { return reinterpret_cast<KeyType *>(this->start); }

        /*
         * KeyBegin() - The start pointer of const KeyType array
         */
        inline const KeyType *KeyBegin() const { return reinterpret_cast<const KeyType *>(this->start); }
         ...
         */
        inline KeyType *KeyEnd() { return reinterpret_cast<KeyType *>(this->extra_data); }

        /*
         * KeyEnd() - The end pointer of const KeyType array
         */
        inline const KeyType *KeyEnd() const { return reinterpret_cast<const KeyType *>(this->extra_data); }

        /*
         * NodeIDBegin() - The start pointer of NodeID array
         */
        inline NodeID *NodeIDBegin() { return this->extra_data; }

        /*
         * NodeIDBegin() - The start pointer of const NodeID array
         */
        inline const NodeID *NodeIDBegin() const { return this->extra_data; }

        /*
         * NodeIDEnd() - End pointer of NodeID array
         */
        inline NodeID *NodeIDEnd() { return reinterpret_cast<NodeID *>(this->end); }

        /*
         * NodeIDEnd() - End pointer of const NodeID array
         */
        inline const NodeID *NodeIDEnd() const { return reinterpret_cast<const NodeID *>(this->end); }
         ...
         */
        inline void WriteItem(int index, const KeyNodeIDPair &knp) { WriteItem(index, knp.first, knp.second); }

        /*
         * WriteItem() - Writes item from another KeyType and NodeID array
         *               (most likely another InnerNode instance)
         */
        inline void WriteItem(int index, int count, const KeyType *key_p, const NodeID *node_id_p)
        {
        ...
        int GetSize() const
        {
            ...
            assert((NodeIDEnd() - NodeIDBegin()) == (KeyEnd() - KeyBegin()));
         ...
         */
        inline int LowerBound(int start_index, int end_index, const KeyType &search_key,
                              const KeyComparator &key_cmp_obj) const
        {
            ...
            const KeyType *it =
                std::lower_bound(KeyBegin() + start_index, KeyBegin() + end_index, search_key, key_cmp_obj);
         ...
         */
        inline int UpperBound(int start_index, int end_index, const KeyType &search_key,
                              const KeyComparator &key_cmp_obj) const
        {
            assert(start_index >= 0 && start_index <= GetSize());
            assert(end_index >= 0 && end_index <= GetSize());
            ...
            const KeyType *it =
                std::upper_bound(KeyBegin() + start_index, KeyBegin() + end_index, search_key, key_cmp_obj);
        ...
        InnerNode *GetSplitSibling() const
        {
            ...
            InnerNode *inner_node_p = InnerNode::Get(0,                          // Depth
    ...
    public:
         ...
         */
        LeafNode(int p_depth, int p_item_count, const KeyNodeIDPair &p_low_key, const KeyNodeIDPair &p_high_key)
            : LeafBaseType{NodeType::LeafType, p_depth, p_item_count, p_low_key, p_high_key}
        {
        }
         ...
         */
        inline KeyValuePair *Begin() { return reinterpret_cast<KeyValuePair *>(this->start); }

        inline const KeyValuePair *Begin() const { return reinterpret_cast<const KeyValuePair *>(this->start); }

        /*
         * End() - Returns an end iterator that is similar to the one for vector
         */
        inline KeyValuePair *End() { return reinterpret_cast<KeyValuePair *>(this->end); }

        inline const KeyValuePair *End() const { return reinterpret_cast<const KeyValuePair *>(this->end); }
         ...
         */
        inline const KeyValuePair *REnd() { return reinterpret_cast<const KeyValuePair *>(this->start) - 1; }

        inline const KeyValuePair *REnd() const { return reinterpret_cast<const KeyValuePair *>(this->start) - 1; }
         ...
         */
        inline void PushBack(const KeyValuePair *copy_start_p, const KeyValuePair *copy_end_p)
        {
         ...
         */
        inline static LeafNode *Get(int p_depth, int p_item_count, const KeyNodeIDPair &p_low_key,
                                    const KeyNodeIDPair &p_high_key)
        {
            ...
            LeafNode *leaf_node_p = static_cast<LeafNode *>(
                LeafBaseType::Get(leaf_node_size, NodeType::LeafType, p_depth, p_item_count, p_low_key, p_high_key));
        ...
        LeafNode *GetSplitSibling(const BwTree *t) const
        {
            ...
            LeafNode *leaf_node_p =
                LeafNode::Get(0, sibling_size, std::make_pair(split_key, ~INVALID_NODE_ID), this->GetHighKeyPair());
...
public:
    ...
    BwTree(bool start_gc_thread = true, KeyComparator p_key_cmp_obj = KeyComparator{},
        ...
        : BwTreeBase(),
          ...
          key_cmp_obj{p_key_cmp_obj}, key_eq_obj{p_key_eq_obj}, key_hash_obj{p_key_hash_obj},

          // Value equality checker and hasher
          value_eq_obj{p_value_eq_obj}, value_hash_obj{p_value_hash_obj},

          // key-node ID pair cmp, equality checker and hasher
          key_node_id_pair_cmp_obj{this}, key_node_id_pair_eq_obj{this}, key_node_id_pair_hash_obj{this},

          // key-value pair cmp, equality checker and hasher
          key_value_pair_cmp_obj{this}, key_value_pair_eq_obj{this}, key_value_pair_hash_obj{this},
          ...
          insert_op_count{0}, insert_abort_count{0}, delete_op_count{0}, delete_abort_count{0}, update_op_count{0},
    ...
    size_t FreeNodeByPointer(const BaseNode *node_p)
    {
        ...
        while (1)
        {
            ...
            case NodeType::InnerType: {
                ...
                for (const NodeID *it = inner_node_p->NodeIDBegin(); it != inner_node_p->NodeIDEnd(); it++)
                {
    ...
    void DebugReplaceNodeIDRecursive(BaseNode *node_p)
    {
        ...
        if (node_p->IsOnLeafDeltaChain() == true)
        {
            ...
            {
                fprintf(stderr, "[Replace NodeID] Unexpected leaf node type: %d\n", static_cast<int>(type));
                exit(1);
            }
        }
        else
        {
            ...
            {
                fprintf(stderr, "[Replace NodeID] Unexpected inner node type: %d\n", static_cast<int>(type));
                exit(1);
            }

            InnerNode *inner_node_p = static_cast<InnerNode *>(node_p);
            for (NodeID *p = inner_node_p->NodeIDBegin(); p != inner_node_p->NodeIDEnd(); p++)
            {
     ...
     * argument list
     */
    int DebugConsolidateAllRecursive(NodeID node_id, int *inner_depth_total, int *leaf_depth_total,
                                     int *inner_node_total, int *leaf_node_total, int *inner_size_total,
                                     int *leaf_size_total, size_t *inner_alloc_total, size_t *inner_used_total,
                                     size_t *leaf_alloc_total, size_t *leaf_used_total)
    {
        const BaseNode *node_p = GetNode(node_id);
        NodeType type = node_p->GetType();
        ...
        if (node_p->IsOnLeafDeltaChain() == true)
        {
            if (type == NodeType::LeafSplitType || type == NodeType::LeafMergeType || type == NodeType::LeafRemoveType)
            {
            ...
            LeafNode::GetAllocationStatistics(&node_p->GetLowKeyPair(), leaf_alloc_total, leaf_used_total);
        ...
        {
            if (type == NodeType::InnerSplitType || type == NodeType::InnerMergeType ||
                type == NodeType::InnerRemoveType || type == NodeType::InnerAbortType)
            {
            ...
            InnerNode::GetAllocationStatistics(&node_p->GetLowKeyPair(), inner_alloc_total, inner_used_total);
            ...
            for (NodeID *p = inner_node_p->NodeIDBegin(); p != inner_node_p->NodeIDEnd(); p++)
            {
                ret += DebugConsolidateAllRecursive(
                    *p, inner_depth_total, leaf_depth_total, inner_node_total, leaf_node_total, inner_size_total,
                    leaf_size_total, inner_alloc_total, inner_used_total, leaf_alloc_total, leaf_used_total);
            }
        }
    ...
    void InitNodeLayout()
    {
        ...
        InnerNode *root_node_p = InnerNode::Get(0, 1, first_sep, std::make_pair(KeyType(), INVALID_NODE_ID));
...
// count to be 0
        ...
        LeafNode *left_most_leaf =
            LeafNode::Get(0, 0, std::make_pair(KeyType(), INVALID_NODE_ID), std::make_pair(KeyType(), INVALID_NODE_ID));
...
// they could call CAS and change the key
     ...
     */
    inline NodeID LocateSeparatorByKey(const KeyType &search_key, const InnerNode *inner_node_p, int start_index,
                                       int end_index)
    {
        ...
        int item_index = inner_node_p->UpperBound(start_index, end_index, search_key, key_cmp_obj) - 1;
        assert(item_index >= 0);
    ...
    inline NodeID LocateSeparatorByKeyBI(const KeyType &search_key, const InnerNode *inner_node_p)
    {
        ...
        // Same searching
        int item_index = inner_node_p->UpperBound(1, inner_node_p->GetSize(), search_key, key_cmp_obj) - 1;
        assert(item_index >= 0);
    ...
    NodeID NavigateInnerNode(Context *context_p)
    {
        ...
        int start_index = 1;
        int end_index = static_cast<InnerNode *>(InnerNode::GetNodeHeader(&node_p->GetLowKeyPair()))->GetSize();
        ...
        while (1)
        {
            ...
            case NodeType::InnerType: {
                ...
                // delta chain
                NodeID target_id = LocateSeparatorByKey(search_key, inner_node_p, start_index, end_index);
            ...
            case NodeType::InnerMergeType: {
                ...
                // are travelling on
                start_index = 1;
                end_index = static_cast<InnerNode *>(InnerNode::GetNodeHeader(&node_p->GetLowKeyPair()))->GetSize();
    ...
    NodeID NavigateInnerNodeBI(Context *context_p)
    {
        ...
        while (1)
        {
            ...
            default: {
                bwt_printf("ERROR: Unknown or unsupported node type = %d", static_cast<int>(type));
    ...
    InnerNode *CollectAllSepsOnInner(NodeSnapshot *snapshot_p, int p_depth = 0)
    {
        ...
        InnerNode *inner_node_p =
            InnerNode::Get(p_depth, node_p->GetItemCount(), node_p->GetLowKeyPair(), node_p->GetHighKeyPair());
        ...
        // and values deleted
        int final_item_count = CollectAllSepsOnInnerRecursive(node_p,
    ...
    int CollectAllSepsOnInnerRecursive(const BaseNode *node_p, NodeID low_key_node_id, T &sss,
                                       InnerNode *new_inner_node_p, int index) const
    {
        ...
        while (1)
        {
            ...
            case NodeType::InnerType: {
                ...
                else
                {
                    ...
                    // consistent
                    copy_end_index =
                        inner_node_p->LowerBound(1, inner_node_p->GetSize(), high_key_pair.first, key_cmp_obj);
                }
                ...
                while (1)
                {
                    ...
                    else if (sss_end_flag == true)
                    {
                        ...
                        new_inner_node_p->WriteItem(index, copy_count, inner_node_p->KeyBegin() + copy_start_index,
                    ...
                    const KeyType &inner_head_key = inner_node_p->KeyBegin()[copy_start_index];

                    if (key_cmp_obj(inner_head_key, sss.GetFront()->item.first) == true)
                    {
                        // If array element is less than data node list element
                        new_inner_node_p->WriteItem(index, inner_node_p->At(copy_start_index));

                        copy_start_index++;
                        index++;
                    }
                    else if (key_cmp_obj(sss.GetFront()->item.first, inner_head_key) == true)
                    {
                        NodeType data_node_type = (sss.GetFront())->GetType();
            ...
            case NodeType::InnerMergeType: {
                ...
                // the new inner node.
                index = CollectAllSepsOnInnerRecursive(merge_node_p->child_node_p, low_key_node_id, sss,
                                                       new_inner_node_p, index);

                index = CollectAllSepsOnInnerRecursive(merge_node_p->right_merge_p, low_key_node_id, sss,
                                                       new_inner_node_p, index);
    ...
    void NavigateLeafNode(Context *context_p, std::vector<ValueType> &value_list)
    {
        ...
        while (1)
        {
...
#else
                if ((copy_start_it != leaf_node_p->End()) && (KeyCmpEqual(search_key, copy_start_it->first)))
                {
                    value_list.push_back(copy_start_it->second);
                }
#endif
            ...
            case NodeType::LeafUpdateType: {
                const LeafUpdateNode *update_node_p = static_cast<const LeafUpdateNode *>(node_p);
                                         ...
                                         std::pair<int, bool> *index_pair_p)
    {
        ...
        while (1)
        {
            ...
            case NodeType::LeafType: {
                ...
                if ((scan_start_it != leaf_node_p->End()) && (KeyCmpEqual(scan_start_it->first, search_key)))
                {
                    index_pair_p->second = true;
    ...
    LeafNode *CollectAllValuesOnLeaf(NodeSnapshot *snapshot_p, LeafNode *leaf_node_p = nullptr)
    {
        ...
        {
            leaf_node_p = LeafNode::Get(0, node_p->GetItemCount(), node_p->GetLowKeyPair(), node_p->GetHighKeyPair());
        }
        ...
        // key value pairs
        LeafItemBloomFilter delta_set{delta_set_data_p, key_value_pair_eq_obj, key_value_pair_hash_obj};
#else
        // For unique keys this is simpler because
        // we just use key type to dedup delta records
        LeafItemBloomFilter delta_set{delta_set_data_p, key_eq_obj, key_hash_obj};
#endif
    ...
    void CollectAllValuesOnLeafRecursive(const BaseNode *node_p, T &sss, LeafItemBloomFilter &delta_set,
        ...
        while (1)
        {
            ...
            case NodeType::LeafType: {
                ...
                else
                {
                    ...
                    copy_end_it =
                        std::lower_bound(leaf_node_p->Begin(), leaf_node_p->End(),
     ...
     */
    inline bool PostInnerInsertNode(Context *context_p, const KeyNodeIDPair &insert_item,
                                    const KeyNodeIDPair &next_item, int location)
    {
     ...
     * NOTE: This function may abort
     */
    inline bool PostInnerDeleteNode(Context *context_p, const KeyNodeIDPair &delete_item,
                                    const KeyNodeIDPair &prev_item, const KeyNodeIDPair &next_item, int location)
    {
        NodeSnapshot *parent_snapshot_p = GetLatestParentNodeSnapshot(context_p);
    ...
    before_switch:
        ...
        case NodeType::InnerRemoveType: {
            ...
            {
                ret =
                    PostLeafMergeNode(left_snapshot_p, &merge_key, merge_right_branch, deleted_node_id, &merge_node_p);
            }
            else
            {
                ret =
                    PostInnerMergeNode(left_snapshot_p, &merge_key, merge_right_branch, deleted_node_id, &merge_node_p);
            }
        ...
        case NodeType::LeafMergeType: {
            ...
            const NodeID *found_node_id_p = NavigateInnerNode(parent_snapshot_p, delete_item_p->first, &location);
...
// constructor
                ...
                const KeyNodeIDPair second_item = std::make_pair(KeyType(), INVALID_NODE_ID);
            ...
            else
            {
                ...
                const NodeID *found_node_id_p = NavigateInnerNode(parent_snapshot_p, insert_item_p->first, &location);
     ...
     */
    const NodeID *NavigateInnerNode(NodeSnapshot *snapshot_p, const KeyType &search_key, int *location)
    {
        // Save some keystrokes
        const BaseNode *node_p = snapshot_p->node_p;
        ...
        assert(node_p->GetNextNodeID() == INVALID_NODE_ID || KeyCmpLess(search_key, node_p->GetHighKey()));
        ...
        while (1)
        {
            ...
            case NodeType::InnerType: {
                ...
                int item_index =
                    inner_node_p->LowerBound(start_index, inner_node_p->GetSize(), search_key, key_cmp_obj);
                ...
                *location = item_index;
                ...
                else if (KeyCmpEqual(inner_node_p->KeyBegin()[item_index], search_key) == false)
                {
    ...
    inline NodeID FindLeftSibling(const KeyType &search_key, NodeSnapshot *snapshot_p)
    {
        ...
        while (1)
        {
            ...
            case NodeType::InnerType: {
                ...
                auto index1 = inner_node_p->UpperBound(1, end_index, search_key, key_cmp_obj) - 1;
                assert(index1 >= 0);
                ...
                while (counter < 2)
                {
                    ...
                    const KeyType &inner_head_key = inner_node_p->KeyBegin()[index1];
                           ...
                           NodeID deleted_node_id, const BaseNode **node_p_p)
    {
        ...
        const LeafMergeNode *merge_node_p = LeafInlineAllocateOfType(LeafMergeNode, merge_branch_p, *merge_key_p,
                                                                     merge_branch_p, deleted_node_id, node_p);
    ...
    bool InsertInPlace(const KeyType &key, const ValueType &value)
    {
        ...
        *((std::pair<KeyType, ValueType> *)move_start_p) = std::make_pair(key, value);
                ...
                const ValueType &new_value)
    {
        ...
        while (1)
        {
            ...
            if (item_p != nullptr)
            {
                ...
                const LeafUpdateNode *update_node_p = LeafInlineAllocateOfType(
                    LeafUpdateNode, node_p, key, item_p->second, new_value, node_p, index_pair);

                bool ret = InstallNodeToReplace(node_id, update_node_p, node_p);
                if (ret == true)
                {
                    break;
                }
            ...
            else
            {
                ...
                const LeafInsertNode *insert_node_p =
                    LeafInlineAllocateOfType(LeafInsertNode, node_p, key, new_value, node_p, index_pair);
    ...
    bool Delete(const KeyType &key, const ValueType &value)
    {
        ...
        while (1)
        {
            ...
            const LeafDeleteNode *delete_node_p = LeafInlineAllocateOfType(LeafDeleteNode, node_p, key,
                                                                           // NOTE: Here we should use the actual value
                                                                           // rather than the random value passed to
                                                                           // this function FOR PRIMARY KEY VARIANT
                                                                           item_p->second, node_p, index_pair);
     ...
     */
    void GetValueNoMappingTable(const KeyType &search_key, std::vector<ValueType> &value_list)
    {
        // printf("Entering\n");
        // EpochNode *epoch_node_p = epoch_manager.JoinEpoch();
        ...
        while (current_node_p->IsOnLeafDeltaChain() == false)
        {
            ...
            current_node_p = reinterpret_cast<const BaseNode *>(
                LocateSeparatorByKey(search_key, inner_node_p, 1, inner_node_p->GetSize()));
        }
        ...
        auto it = std::lower_bound(start_p, end_p, std::make_pair(search_key, ValueType{}), key_value_pair_cmp_obj);
        // printf("After lower bound\n");
        if ((it != leaf_node_p->End()) && (KeyCmpEqual(search_key, it->first)))
        {
    ...
    public:
        ...
        inline static IteratorContext *Get(BwTree *p_tree_p, const BaseNode *node_p)
        {
            ...
            new (ic_p->GetLeafNode())
                LeafNode{node_p->GetDepth(), node_p->GetItemCount(), node_p->GetLowKeyPair(), node_p->GetHighKeyPair()};
